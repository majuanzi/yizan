...
//axios.defaults.baseURL ='http://192.168.11.101:8080/carebussinessweb/';//尹清亮电脑
//axios.defaults.baseURL ='http://192.168.9.0:11000/carebussinessweb/'; //姜松电脑
//axios.defaults.baseURL = 'http://218.245.7.26/carebussinessweb/';//演示部署服务器
...
export function postSafe(url, data = {}) {
	return new Promise((resolve, reject) => {
		axios({
			method: "post",
			url: url,
			data: qs.stringify(data),
			headers: { "token": window.sessionStorage.getItem('sessionToken') }
		}).then(res => {
			if (res.data.resultcode == 1 || res.data.result == 1) {
				warningNum = 0;
				resolve(res.data);
